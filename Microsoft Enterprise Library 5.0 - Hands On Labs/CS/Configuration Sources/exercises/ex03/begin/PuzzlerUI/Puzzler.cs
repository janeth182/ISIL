...
namespace PuzzlerUI
{
    ...
    public class Puzzler : System.Windows.Forms.Form
    {
        ...
        {
            string confirurationSourceText = GetConfigurationSourceText();
            Text = string.Format("Puzzler - {0}", confirurationSourceText);
            lblDictionaryInfo.Text = PuzzlerService.DictionaryService.GetDictionaryInfo();
        }
        ...
        private void txtWordToCheck_TextChanged(object sender, System.EventArgs e)
        {
            ...
            if (PuzzlerService.DictionaryService.CheckSpelling(txtWordToCheck.Text))
            {
        ...
        {
            PuzzlerService.DictionaryService.AddWord(txtWordToCheck.Text);
            errorProvider1.SetError(txtWordToCheck, "");
        }
        ...
        private string GetConfigurationSourceText()
        {
            var configurationSourceSection = (ConfigurationSourceSection)ConfigurationManager.GetSection(ConfigurationSourceSection.SectionName);
            if (configurationSourceSection == null)
            {
                //if there is no configuration source section, 
                // the application domain's configuration is used.
            ...
            var selectedSourceElement = configurationSourceSection.Sources.Where(x => x.Name == selectedSource).FirstOrDefault();
            if (selectedSourceElement == null)
            {
                //a selected source was specified, but couldnt be located.
                // this is invalid configuration.
                return string.Format("Selected Source {0} - unable to locate source in <sources/> collection", selectedSource);
            }
        
            
            if (selectedSourceElement is SystemConfigurationSourceElement)
            {
                //the selected source is a SystemConfigurationSource, 
                // the application domain's configuration is used.
                return Path.GetFileName(AppDomain.CurrentDomain.SetupInformation.ConfigurationFile);
            }
            if (selectedSourceElement is FileConfigurationSourceElement)
            {
                //the selected source is a FileConfigurationSource, 
                // the specified file is used.
                return Path.GetFileName(((FileConfigurationSourceElement) selectedSourceElement).FilePath);
            }

            //the selected source is something different, 
            // the resource used depends on the source's implementation and configuration.
            return string.Format("Selected Source {0} - {1}", selectedSource, selectedSourceElement.Type.Name);
